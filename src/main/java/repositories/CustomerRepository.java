...
/*	@Query("select min(g.signing.customer.size), avg(g.signing.customer.size), max(g.signing.customer.size), stddev(g.signing.customer.size) from Gym g")
	...
	/*
	@Query("select c.name from Customer c where (select count(s1) from Customer c1 join c.signing s1) >= ALL(select count(s2) from Customer c2 join c.signing s2  group by c2)")
	String findMaxCustomer();
	*/
}
